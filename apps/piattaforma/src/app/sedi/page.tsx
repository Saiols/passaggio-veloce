...
import { createSedeAction, suspendSedeAction, reactivateSedeAction } from './actions';
...
export default async function SediPage({
  ...
  async function handleCreate(formData: FormData): Promise<void> {
    'use server';
    const res = await createSedeAction(formData);
    const { redirect } = await import('next/navigation');
    redirect(res.ok ? '/sedi?saved=1' : `/sedi?error=${encodeURIComponent(res.error)}`);
  }
    ...
    <AppShell session={session} activePath="/sedi">
      <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-6 sm:py-10">
        ...
        <section className="rounded-2xl border border-pv-slate-200 bg-white p-6">
          ...
          <form action={handleCreate} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Field name="nome" label="Nome sede *" />
            <Field name="indirizzo" label="Indirizzo *" />
            <Field name="civico" label="Civico" />
            <Field name="citta" label="Città *" />
            <Field name="cap" label="CAP *" />
            <Field name="provincia" label="Provincia (sigla) *" />
            <Field name="telefono" label="Telefono" />
            <Field name="email" label="Email operativa" />
            <Field name="iban" label="IBAN dedicato (opzionale)" full />
...
function Field({ name, label, full }: { name: string; label: string; full?: boolean }) {
  return (
    <label className={`flex flex-col gap-1 ${full ? 'sm:col-span-2' : ''}`}>
      <span className="text-[12px] font-semibold text-pv-slate-600">{label}</span>
      <input
        name={name}
        className="rounded-[10px] border-[1.5px] border-pv-slate-200 bg-white px-3 py-2 text-[14px] text-pv-navy-900 focus:border-pv-navy-600 focus:outline-none focus:shadow-[var(--pv-ring-focus)]"
      />
    </label>
  );
}
