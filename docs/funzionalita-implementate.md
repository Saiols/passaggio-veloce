...
- **[CLIENTI] Distribuzione a raggio crescente + pool cumulativo** (`lib/distribuzione/tick.ts`, `anelli.ts`, `config.ts`, **aggiornata 2026-07-25**, vedi `superpowers/specs/2026-07-25-distribuzione-round-config-design.md`): selezione per **distanza in linea d'aria (Haversine)** dalle coordinate del luogo di consegna scelto dal broker via autocomplete (`Pratica.lat/lng`), NON per provincia. Stato unico `IN_DISTRIBUZIONE`. Il raggio parte da **1 km** e cresce di **1 km per round** fino a **10 km**; la durata di un round si misura in **minuti** (da **1 a 60**, default **60**). Superato il raggio massimo senza accettazione la pratica è **"zona non coperta"** (N52 al broker — **una sola volta per ciclo**, vedi ripresa sotto; le PENDING restano accettabili). Tutti e quattro i parametri sono editabili da `/admin/distribuzione` (raggio iniziale, passo, raggio massimo in km; durata round in **minuti**). **Tutte** le sedi idonee nell'anello ricevono l'assegnazione simultaneamente (nessun cap, nessuna selezione per ranking). **Pool cumulativo**: le sedi dei round precedenti restano `PENDING` e accettabili per sempre (nessun countdown di risposta). **Un round senza nemmeno un'agenzia non fa attendere**: il raggio salta subito al primo anello non vuoto, nello stesso tick, e la prima notifica parte già al submit. Il **round avanza solo su una notifica reale**, quindi `round N ≈ (N-1) × durata round`. L'attesa fra un round e l'altro è contata in **minuti lavorativi**: i minuti fuori fascia, nei giorni spenti e nei festivi non consumano il round, così ogni cerchio ha la sua finestra piena anche se la pratica è arrivata di notte. Il **cron gira ogni minuto** (`vercel.json`, `/api/jobs/distribuzione-tick`), quindi la cadenza reale può variare di un minuto rispetto alla durata impostata. Sedi senza coordinate escluse. **Prima agenzia che accetta vince** (lock `FOR UPDATE` sulla pratica).
- **[CLIENTI] Ripresa dalla zona non coperta** (`lib/distribuzione/tick.ts`, **2026-07-26**): una pratica marcata "zona non coperta" **non è un vicolo cieco**. Il cron continua a considerarla e, appena un'agenzia idonea si registra nella sua zona, la pratica **riparte dal raggio iniziale** (le agenzie nuove possono essere ovunque entro il massimo, quindi si riapre il ventaglio dalla più vicina): `zonaNonCopertaAt` torna a `null`, partono le N6 alle agenzie e nessuna notifica al broker (non c'è nulla da smentire). Se il tentativo fallisce — l'agenzia non accetta — il motore ri-dichiara la zona non coperta **senza rimandare la N52**: al broker la notifica arriva una sola volta per ciclo di distribuzione. L'anzianità della pratica vive in una colonna dedicata (`Pratica.zonaNonCopertaPrimaAt`), che la ripresa **non** azzera: in `/admin/monitoraggio` una pratica ferma da settimane resta ferma da settimane anche dopo un tentativo andato a vuoto. Solo il **ricircolo dopo revoca admin** (nuovo ciclo) azzera l'anzianità. I tentativi di ripresa sono throttlati dal gate della durata round: uno ogni `durata round` minuti lavorativi, non uno al minuto.
- **Round di accettazione (solo admin)** (`Pratica.roundAccettazione`, `lib/distribuzione/statistiche.ts`): all'accettazione si registra in che round la pratica è stata presa. Serve alla **media** mostrata su `/admin/distribuzione` ("entro quanto vengono accettate") e compare nel dettaglio pratica, nella lista `/admin/pratiche` e nell'export CSV. Non è mai esposto alle agenzie. Le pratiche assegnate a mano dall'admin restano fuori dal campione.
- **Calendario piattaforma** (`lib/distribuzione/calendario.ts` + `orario-piattaforma.ts`, **aggiornato 2026-07-26**, vedi `docs/superpowers/specs/2026-07-26-distribuzione-round-minuti-orari-design.md`): l'espansione del raggio gira solo dentro il calendario della piattaforma, **configurabile da `/admin/distribuzione`**. Non è più una fascia unica lun-ven: c'è **una fascia oraria per ogni giorno della settimana**, con il giorno attivabile/disattivabile singolarmente (default: LUN-VEN 09:00-19:00, weekend spento), più un elenco di **giorni festivi** editabile (data + nome), in cui il motore è fermo indipendentemente dal giorno della settimana. Tutti i confronti di orario sono fatti in fuso **Europe/Rome** (il processo su Vercel gira in UTC). Fuori fascia il motore è in pausa e riprende dal raggio salvato; il **primo** round ignora il calendario (parte a qualsiasi ora).
...
- **[CLIENTI] Inbox agenzia** (`/inbox`): accetta o rifiuta, senza scadenza — un'assegnazione non accettata resta disponibile mentre il raggio si allarga ad altre agenzie. Esiti assegnazione: PENDING/ACCETTATA/RIFIUTATA/TIMEOUT (legacy)/ASSEGNATA_ALTRO/REVOCATA_ADMIN.
...
- **CRM** (`/admin/crm/**`, ruoli AD/CTO/CFO/SALES_MANAGER/SALES):
  - **Contatti** pipeline lead S0–S10 (anagrafica, funnel, dati chiamate, tracking pixel, aggancio all'azienda registrata), import CSV, assegnazione sales (SALES vede solo i propri).
  - **Riconciliazione CRM ↔ aziende registrate** (`lib/crm/match/**`, **dal 2026-07-27**, spec `crm-spec-implementativa.md` §12): riconosce quali righe della lista importata sono aziende **e sedi** già registrate. Serve una **prova forte** (P.IVA, email/PEC, telefono/WhatsApp) — nome, indirizzo, città e CAP non bastano mai da soli — e a categoria discorde serve un secondo indizio identificante. Vince il punteggio più alto ("più campi uguali"); a pari merito fra madre e sua sede vince la **sede**. **Ogni sede è un'identità a sé**: un'azienda con 3 sedi può agganciare fino a 4 righe. Lo stato del funnel è allineato allo **storico reale** delle pratiche (0 firmate → S7, 1 → S8, ≥2 → S9), **solo in salita**, **S10 mai** assegnato dal motore. Tre canali: registrazione, cron `crm-sync`, pagina `/admin/crm/riconciliazione` (anteprima + Applica, permesso CRM full). I canali **automatici non applicano le proposte ambigue** (pari merito fra aziende diverse): restano alla pagina admin, dove decide una persona. **Non esiste uno sgancio** — cancellare il contatto sbagliato non ferma il ciclo, torna libero e la notte dopo aggancia il secondo migliore. 🚩 Sostituisce la cascata `email → tel → P.IVA` descritta nei paper di aprile, che non agganciava **mai** nulla (0 contatti su 19.103).
  - **Contatti operativi** (venditori/acquirenti deduplicati dalle pratiche).
  - **Sales** (`CrmSalesAgent` per Vapi: voce/accento/lingua/prompt/script/Q&A; `CrmCampaign` outbound con filtri, orari, max tentativi). Integrazione Vapi reale in CRM-H (account esterno). Il target di una campagna **esclude sempre** i contatti già agganciati a un'azienda registrata, qualunque filtro sia stato scelto (il bot propone l'iscrizione: chiamare un cliente attivo è il danno peggiore); il modale di lancio dice quanti sono stati esclusi. Le assegnazioni delle campagne **già lanciate** non vengono ripulite a posteriori.
...
- **Matching CRM**: se un documento dice che l'aggancio CRM ↔ piattaforma avviene "in cascata email → telefono → P.IVA", quel documento è di aprile 2026 e descrive un CRM esterno mai realizzato. La regola vera è nel bullet Riconciliazione della §5 e, per esteso, in `crm-spec-implementativa.md` §12. Che una riga del CRM sia agganciata a un'azienda è un **dato interno**: non va mai riferito all'interlocutore.
- **Guida operativa per gli agenti** (sales vocale e chatbot): `ecosistema-crm-ai.md` §10 — cosa significano oggi S7/S8/S9, come si comporta il bot se emerge che l'azienda è già iscritta, e la checklist dei testi da aggiornare a mano da `/admin/crm/sales` e `/admin/crm/chatbot` (i prompt vivono nel DB, non nei docs).
