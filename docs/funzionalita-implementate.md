...
- **[CLIENTI] Engine documenti** (`lib/documenti/engine.ts`, `calcolaDocumentiRichiesti()`): albero decisionale che determina la lista esatta di documenti da 8-9 variabili. Esiti: `OK` (lista) / `BLOCCO` (motivo+soluzione) / `INPUT_INCOMPLETO`. Pre-2015 → +Certificato di Proprietà. Comodato attivo → BLOCCO finché non revocato. Procura/successione/minore → documenti extra.
