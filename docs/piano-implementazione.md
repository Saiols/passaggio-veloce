...
> ⚠️ **SUPERATO (ultimo aggiornamento 2026-07-26)** — il motore a 3 round fissi (500/750/1000 m @ 4h) descritto qui sotto è stato **sostituito**. Motore attuale: raggio a **espansione per round**, misurato in **linea d'aria** (Haversine), default **1 km → +1 km per round → max 10 km**, con **durata round in minuti (1-60, default 60)**; tutti i parametri **editabili da `/admin/distribuzione`**; **calendario della piattaforma per giorno + festivi** (gli orari dichiarati dalle agenzie NON hanno effetto sulla distribuzione), con l'attesa fra round misurata in **minuti lavorativi** — notte, giorni chiusi e festivi valgono zero; **cron ogni minuto**; **notifiche cumulative eterne** (niente countdown 4h/TIMEOUT); un **round vuoto non fa attendere** (salto immediato al primo anello non vuoto, già al submit); il **primo round parte sempre**, anche fuori orario; stato unico **`IN_DISTRIBUZIONE`**; **zona non coperta** (email broker N52) al posto dell'escalation, con **ripresa automatica** se un'agenzia idonea si registra in zona (la N52 parte una sola volta per ciclo); **card Copertura** admin sul dettaglio pratica, che spiega perché una sede non è stata contattata; **accept con lock `FOR UPDATE`** (primo atomico); **round di accettazione** salvato sulla pratica (dato admin-only, media su `/admin/distribuzione`). L'**auto-sospensione no-show è stata rimossa** (le notifiche non scadono più → niente trigger TIMEOUT). Vedi changelog **A13/A14/A15** e `superpowers/specs/2026-07-26-distribuzione-round-minuti-orari-design.md`. Quanto descritto sotto **non riflette più il codice attuale** (lasciato per storico).
...
| 4 Distribuzione + agenzia | 100% | Engine **a round configurabili (A15, 2026-07-26): raggio in linea d'aria 1 km → +1 km/round → max 10 km, durata round in MINUTI (1-60, default 60), tutti i parametri editabili da /admin/distribuzione** + **calendario della piattaforma per giorno + festivi** (gli orari dichiarati dalle agenzie non hanno effetto) + **attesa misurata in minuti lavorativi** (la notte e i giorni chiusi non contano) + **cron al minuto** + round vuoto → avanti subito + notifiche cumulative eterne + zona-non-coperta (N52) con **ripresa automatica** quando entra un'agenzia idonea + **card Copertura admin** (perché una sede non è stata contattata) + accept FOR UPDATE + **round di accettazione (media admin)**; stato unico `IN_DISTRIBUZIONE`; auto-suspend no-show rimosso. Sostituisce v2 stradale (A13), v1 3-round (A12) e comune/limitrofi/ranking (A3) |
...
- `apps/piattaforma/vercel.json` con i cron paths (**distribuzione-tick ogni minuto dal 2026-07-26**, send-solleciti 9:00, process-fee-scheduled 5:00, process-payouts 1:30, trigger-auto-payout 1:00, crm-sync 2:00, e gli altri aggiunti in seguito: affiliation-monthly-recap, purge-deleted-team-users, purge-deleted-documenti, preavviso-visura)
...
**A13. ✅ DONE — [2026-07-21] Distribuzione v2: raggio incrementale + distanza stradale (sostituisce v1 3-round A12)** — ⚠️ la parte **distanza stradale** è stata **rimossa** da A14 (2026-07-25): il raggio è tornato in linea d'aria e `lib/geo/road-distance.ts`, i provider Google/mock e la tabella `road_distance_cache` non esistono più.
...
**A15. ✅ DONE — [2026-07-26] Distribuzione: round in minuti, calendario piattaforma con festivi, copertura diagnosticabile, ripresa zona non coperta**
- **Durata round in MINUTI (1-60, default 60)** al posto delle ore: il DB memorizzava già i minuti, sparisce la conversione nel form. **Cron `distribuzione-tick` da `*/10` a `* * * * *`** (ogni minuto): sotto i 10 minuti nessuna durata sarebbe stata rispettabile. Grazia sul gate ridotta da 1 min a 0,2 min (12 s), che assorbe il jitter dello scheduler senza dimezzare i round corti
- **Calendario della piattaforma per giorno + festivi**, editabili da `/admin/distribuzione`: due colonne `JSONB` (`orariSettimana`, `festivi`) sostituiscono `orarioInizio`/`orarioFine`/`giorni`. Gli orari dichiarati dalle **agenzie** non hanno (e non avevano già) alcun effetto sulla distribuzione. Validazione: almeno un giorno attivo, `fine > inizio` sui giorni attivi. Seed dei 16 festivi nazionali futuri (Pasquetta col computus) + avviso nel pannello quando il calendario si esaurisce entro 60 giorni
- **Attesa fra round in MINUTI LAVORATIVI** (`minutiLavorativiTra`, con early-exit sul cap): notte, giorni chiusi e festivi valgono zero. Una pratica inviata alle 00:20 con round da 60 min fa partire il secondo cerchio alle 10:00, non alle 09:00 — ogni cerchio ha la sua finestra piena per rispondere. Il **primo round continua a partire sempre**, a qualsiasi ora e in qualsiasi giorno
- **Card "Copertura"** admin sul dettaglio pratica (`lib/distribuzione/copertura.ts`): elenca le agenzie entro il raggio massimo e per ciascuna dice se è contattata (round + esito), in attesa del turno, o **esclusa e perché** (`COORDINATE_MANCANTI`, `VISURA_SCADUTA`, `SEDE_SOSPESA`, `AZIENDA_SOSPESA`, `BLOCCO_PAGAMENTO`, `REVOCATA_ADMIN`). Nasce dal caso reale di una pratica di Corsico mai arrivata all'unica agenzia di Assago: quei filtri escludevano in silenzio. Le sedi senza coordinate sono elencate a parte come "posizione ignota", senza distanze inventate
- **Ripresa della zona non coperta**: `zonaNonCopertaAt` non è più definitivo. Il cron non filtra più via quelle pratiche e, se nel frattempo si registra un'agenzia idonea, la pratica riparte da `primoAnello` (il round prosegue, non riparte da 1). Nuova `Pratica.zonaNonCopertaPrimaAt`, che la ripresa **non** azzera: serve a mandare la N52 al broker **una sola volta per ciclo** e a non azzerare il contatore "ferma da N giorni" del monitoraggio. `markZonaNonCoperta` scrive anche `ultimaEspansioneAt` per throttlare i tentativi di ripresa a uno ogni durata-round invece di uno al minuto
- ⚠️ **Prereq deploy — le due finestre di migration NON sono eseguibili con `db:deploy`** (applicherebbe anche il DROP): additive `20260726120000_distribuzione_calendario` e `20260726140000_zona_non_coperta_prima` a mano su Neon **prima** del push (con `prisma migrate resolve --applied`), verifica che la riga `singleton` esista (gli `UPDATE` sono no-op silenziosi e il seed dei festivi andrebbe perso), poi il codice, poi `20260726130000_drop_orario_legacy`
- 12 task SDD (implementer + reviewer per-task, con fix loop) + review whole-branch + fix wave. Suite 2247/0, tsc 0
- Spec: `docs/superpowers/specs/2026-07-26-distribuzione-round-minuti-orari-design.md` · Plan: `docs/superpowers/plans/2026-07-26-distribuzione-round-minuti-orari.md`

**A14. ✅ DONE — [2026-07-25] Distribuzione: raggio in linea d'aria, round configurabili, metrica di accettazione**
- **Raggio in linea d'aria**: `candidatiEntro` usa solo Haversine. **Rimossi** `lib/geo/road-distance.ts`, i provider Google/mock e i loro test; modello `RoadDistanceCache` droppato. Nessuna chiamata di rete nel tick (la struttura candidati-fuori-tx resta: serve al compare-and-set, non più al timeout)
- **Round = ordinale del batch notificato**, non del raggio: gli anelli vuoti fanno avanzare il raggio ma non il round e non consumano tempo → `round N ≈ (N-1) × durata round`. `PraticaAssegnazione.round` torna significativo (era cablato a 1); nuova `Pratica.roundCorrente`
- **Round vuoto → avanti subito anche al submit**: `avviaRound1ForPratica` cerca i candidati fino al raggio **massimo** e applica il nuovo `primoAnello()` — la prima notifica parte al submit al primo anello non vuoto invece di aspettare il tick del cron. Nessuna sede entro il massimo → **zona non coperta immediata** (N52 al submit)
- **`Pratica.roundAccettazione`** scritto in `accettaPratica` sotto il row lock `FOR UPDATE`. Visibile SOLO admin: dettaglio pratica, colonna in `/admin/pratiche`, colonna CSV in `/api/admin/dashboard/export`, **media + istogramma** su `/admin/distribuzione` (`lib/distribuzione/statistiche.ts`). Le assegnazioni manuali (`ESCALATION_ROUND = 99`, spostata in `lib/distribuzione/constants.ts`) non scrivono il round → fuori dal campione. Il ricircolo dopo revoca azzera `roundCorrente`/`roundAccettazione`
- **`round` rimosso dal payload e dal template N6**: il dato è admin-only, dirlo all'agenzia significherebbe dirle che non era la prima scelta
- **`/admin/distribuzione` con 4 campi editabili** in km e ore (raggio iniziale, passo, raggio massimo, durata round); persistenza invariata in metri/minuti (`toConfigPersistita`). Nuovi default: **1 km / +1 km / 1 h / max 10 km** (erano 500 m / +200 m / 10 min)
- ⚠️ Prereq deploy in **due finestre**: `20260725120000_distribuzione_round_config` (additiva: 2 colonne + `@default` + **`UPDATE` esplicito del singleton**, perché `SET DEFAULT` non tocca una riga esistente) va **PRIMA** del push; `20260725130000_drop_road_distance_cache` va **DOPO** il deploy — il codice della release precedente legge `road_distance_cache` con una `findMany` senza try/catch, droppare in anticipo romperebbe la creazione pratica. Nessun backfill di `roundAccettazione` (DB prod usa-e-getta). L'env `DISTANCE_PROVIDER` su Vercel non serve più
- Spec: `docs/superpowers/specs/2026-07-25-distribuzione-round-config-design.md`
