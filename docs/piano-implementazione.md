...
> ⚠️ **SUPERATO (ultimo aggiornamento 2026-07-25)** — il motore a 3 round fissi (500/750/1000 m @ 4h) descritto qui sotto è stato **sostituito**. Motore attuale: raggio a **espansione per round**, misurato in **linea d'aria** (Haversine), default **1 km → +1 km per round → max 10 km, un round all'ora**, tutti e quattro i parametri **editabili da `/admin/distribuzione`**; **notifiche cumulative eterne** (niente countdown 4h/TIMEOUT); un **round vuoto non fa attendere** (salto immediato al primo anello non vuoto, già al submit); **pausa notturna** (lun-ven 9-19); stato unico **`IN_DISTRIBUZIONE`**; **zona non coperta** (email broker N52) al posto dell'escalation; **accept con lock `FOR UPDATE`** (primo atomico); **round di accettazione** salvato sulla pratica (dato admin-only, media su `/admin/distribuzione`). L'**auto-sospensione no-show è stata rimossa** (le notifiche non scadono più → niente trigger TIMEOUT). Vedi changelog **A13/A14** e `superpowers/specs/2026-07-25-distribuzione-round-config-design.md`. Quanto descritto sotto **non riflette più il codice attuale** (lasciato per storico).
...
| 4 Distribuzione + agenzia | 100% | Engine **a round configurabili (A14, 2026-07-25): raggio in linea d'aria 1 km → +1 km/round → max 10 km, un round all'ora, tutti i parametri editabili da /admin/distribuzione** + round vuoto → avanti subito + notifiche cumulative eterne + pausa notturna + zona-non-coperta (N52) + accept FOR UPDATE + **round di accettazione (media admin)**; stato unico `IN_DISTRIBUZIONE`; auto-suspend no-show rimosso. Sostituisce v2 stradale (A13), v1 3-round (A12) e comune/limitrofi/ranking (A3) |
