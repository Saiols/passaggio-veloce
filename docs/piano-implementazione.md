...
- ⚠️ Prereq deploy: migration Neon `20260725120000_distribuzione_round_config` **prima** del codice — aggiunge le due colonne, cambia i `@default` **e fa l'`UPDATE` esplicito della riga singleton** (un `SET DEFAULT` non tocca una riga già esistente), droppa `road_distance_cache`. Nessun backfill di `roundAccettazione` (DB prod usa-e-getta). L'env `DISTANCE_PROVIDER` su Vercel non serve più
